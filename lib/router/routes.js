 ...
 * example:http://localhost:3000/searchuser?q=u will return users that their name start with the letter u 
 */
router.get('/searchuser',userModel ,bearer,handleSearch);
router.get('/searchuskill',userModel ,bearer,handleSearchSkill);
router.get('/searchtitle',projectModel,bearer,handleSearchTitle);
...
function handleSearch (req,res){
  let q =req.query.q;
  req.model.search(q).then(data=>{
    res.status(200).json(data);
  }).catch(err=> {
    console.log(err);
  });
}
function handleSearchSkill(req,res){
  let q =req.query.q;
  req.model.searchSkill(q).then(data=>{
...
function handleSearchTitle(req,res){
  let q =req.query.q;
  req.model.search(q).then(data=>{
    res.status(201).json(data);
  }).catch(err=> {
