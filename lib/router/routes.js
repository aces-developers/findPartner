...
const pojectDirctModel=require('../collections-schema/projects/project-model');
const Votes = require('../collections-schema/votes/votes-model');
const userDirectModel=require('../collections-schema/users/user-model')
...
router.get('/proposal/:id', applyModel, bearer, handlePropsal);
...
function deleteProject(req, res) {
  ...
  console.log('req.body._ownerId',req.params);
  console.log('req.body._ownerId',req.query);
  let O = req.query.oid;
...
// return all projects That spacifc user applied on it
 function handleallApply(req, res) {
  ...
  req.model
    ...
    .then((data) =>  {
      //pojectDirctModel.get()
      
      console.log('Item --> ',data)
      let arr=data.map(item=> { return item.projectId})
     console.log('arr --> ',arr)
      //res.status(201).send(data);
      pojectDirctModel.getListProject(arr).then((reapons)=> res.status(201).send(reapons))
...
function handlePropsal(req,res){
  let project_id = req.params.id;
  req.model.getProposal(project_id).then((reapons)=> {
    console.log('reapons',reapons)
    let arr=reapons.map(item=> { return item.userId})
    userDirectModel.getListUser(arr).then((data)=> {
      let arrObj=reapons.map((item,idx)=> { 
        
        return  { "proposal":item.proposal,"username":data[idx].username }
      
      })
      res.status(201).send(arrObj)
    })
