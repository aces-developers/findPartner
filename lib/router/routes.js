...
router.get('/proposal/:id', applyModel, bearer, handleProposal);
  ...
  req.model
    ...
    .then((data) =>  {
     ...
     console.log('arr --> ',arr)
      ...
      pojectDirctModel.getListProject(arr).then((respons)=> res.status(201).send(respons))
...
function handleProposal(req,res){
  ...
  req.model.getProposal(project_id).then((respons)=> {
    console.log('respons',respons)
    let arr=respons.map(item=> { return item.userId})
    console.log('arr : ',arr)
    userDirectModel.getListUser(arr).then((data)=> {
      let arrObj=respons.map((item,idx)=> { 
        console.log('idx : ',idx)
        return  { "proposal":item.proposal,"username":data[idx].username,"email":data[idx].email }
